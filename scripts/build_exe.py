#!/usr/bin/env python3
"""
PyInstaller Build Script for Code Review Assistant
...
class EXEBuilder:
    ...
    def __init__(self, clean: bool = False, debug: bool = False):
        ...
        self.spec_file = self.project_root / "CodeReviewAssistant.spec"
    ...
    def pre_check(self):
        ...
        if not templates_dir.exists():
            raise BuildError(f"Templates directory not found: {templates_dir}")
        print(f"✓ Templates directory found: {templates_dir}")

        # Check languages directory
        languages_dir = resources_dir / "languages"
        if not languages_dir.exists():
            raise BuildError(f"Languages directory not found: {languages_dir}")
        print(f"✓ Languages directory found: {languages_dir}")
    ...
    def post_check(self):
        ...
        # Check EXE exists
        exe_path = self.dist_dir / "CodeReviewAssistant.exe"
        ...
        print()
        print("📦 Next steps:")
        print("   1. Test the EXE: ./dist/CodeReviewAssistant.exe")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Build Code Review Assistant EXE with PyInstaller"
    )
    parser.add_argument(
        "--clean",
        action="store_true",
